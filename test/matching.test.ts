...
describe("filenameParts", function () {
  ...
  it("takes only the first author from an et al. or two-author name", function () {
    assert.equal(
      filenameParts("Scheffer et al. - 2009 - Early-warning.pdf").first,
      "Scheffer",
    );
    assert.equal(
      filenameParts("Arksey and O'Malley - 2005 - Scoping.pdf").first,
      "Arksey",
    );
  });
...
describe("titleScore", function () {
  ...
  it("stays under the reconcile floor for a different paper", function () {
    assert.isBelow(
      titleScore(file, "Alternative stable states in ecology"),
      0.86,
    );
  });
  ...
  it("will not let a short filename match anything that starts the same", function () {
    ...
    assert.isBelow(
      titleScore("Smith - 2020 - The.pdf", "The Theory of Everything"),
      0.86,
    );
  });
