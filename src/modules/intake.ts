...
export type IntakeStatus =
  ...
  | "already has PDF"
  /** The file is a paper's supplementary material, not a paper of its own. */
  | "supplement"
  /** Another file earlier in the same batch is the same paper. */
  | "duplicate"
...
export async function scan(
  ...
  for (const attachment of attachments) {
    ...
    const { record, doi, doiSource, via, reason } = await resolve(
      path,
      filename,
    );
...
function checkFiled(
  ...
  if (via !== "doi" || !record) {
    ...
    status = "cannot verify";
    why =
      "this file is the item's supplementary material, which carries its own DOI";
  } else if (!parentDoi) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (options.dryRun) {
        ...
        result.log.push(
          "  would rename the stored file to Zotero's convention",
        );
        continue;
      }
