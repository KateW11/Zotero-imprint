...
import { beginRun, throwIfStopped } from "../utils/cancel";
...
export async function reconcile(
  ...
  // Folded once for the whole scan rather than once per file per item.
  const foldedTitles = [...items.values()].map((item) => ({
    item,
    folded: normText(item.title),
  }));
  ...
  for (const f of files) {
    ...
    // Parsed once per file rather than once per item: the inner loop below
    // runs over the whole library.
    const parts = filenameParts(f.name);

    if (!hit) {
      let best: LibraryItem | null = null;
      let score = 0;
      const folded = normText(parts.title);
      for (const cand of foldedTitles) {
        // Two titles of very different lengths cannot reach the floor
        // whatever they contain, so the string comparison -- the expensive
        // part, run once per file per item -- is skipped for them.
        if (
          titleScoreCeiling(folded.length, cand.folded.length) < TITLE_FLOOR
        ) {
          continue;
        }
        const s = foldedTitleScore(folded, cand.folded);
        if (s > score) {
          best = cand.item;
          score = s;
        }
      }
      // The title score alone does not decide it. A filename Zotero truncated
      // is a prefix of every title that starts the same way, so the top scorer
      // has to agree on the year or an author before the file counts as
      // accounted for. Refusing here costs a line in "in the folder only",
      // which is a question; accepting wrongly hides a file under another
      // paper and reports the right paper as missing, which is an answer.
      if (score >= TITLE_FLOOR && best && corroborates(parts, best)) hit = best;
    }

    if (!hit) {
      // Last pass: first author and year. Rescues a filename whose title was
      // truncated hard or spelled differently from the item.
      const { first, year } = parts;
...
export function asMarkdown(rep: ReconcileReport, folder: string): string {
  ...
  block(
    `In ${rep.scope}, no copy in the folder`,
    rep.libraryOnly,
    (r) =>
  ...
  block(
    ...
    rep.broken,
    (r) => `[${r.key}] ${r.title} — ${r.filename}`,
  );
  block(
    "Items with no PDF attached",
    rep.noPdf,
    (r) => `[${r.key}] ${r.title}`,
  );
