...
Those two must be different folders. Everything else can wait: the export
...
2. Set **Source** to **PDFs in a folder**
3. Next to **Read from**, click **Use settings folder** — or **Choose…** if you
   want a different folder just this once
4. Click **Scan**

Every file is read and looked up. Nothing has been written yet.

Reading a DOI out of a PDF takes about a second a file, so a large folder takes
a while. **Stop** ends the run after the file it is on and shows you what it
got — the summary says so, because a short list should not read like a clean
one.
...
| **supplement**      | a paper's supplementary material, not a paper — left alone, parent named    |
| **duplicate**       | the same paper as another file in this batch — only the first one is filed  |
| **needs you**       | nothing confident enough to act on. Left alone, with the reason             |

Only **new** and **attach** are ticked to begin with. The other four are the
plugin declining to act, each for a different reason, and the reason is on the
row.
